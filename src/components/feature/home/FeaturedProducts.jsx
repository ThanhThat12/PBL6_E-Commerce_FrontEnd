...
const ProductCard = ({ product, showDiscount = false }) => {
  const {
    ...
    image,
    basePrice,
    price, // giá thấp nhất từ variants hoặc basePrice
    originalPrice,
    discount,
    rating = 0,
    reviewCount = 0,
    soldCount = 0, // Số lượng đã bán
    brand,
    inStock = true,
    badge, // "new", "hot", "sale"
  ...
  // Tính giá hiển thị: ưu tiên variant có giá thấp nhất, nếu không có thì dùng basePrice
  const calculateDisplayPrice = () => {
    if (variants && variants.length > 0) {
      ...
      return {
        ...
        hasVariantDiscount: lowestVariant.price < (basePrice || price),
    ...
    return {
      ...
      originalPrice: originalPrice || (basePrice || price)
    };
  };

  const { displayPrice, hasVariantDiscount, originalPrice: computedOriginalPrice } = calculateDisplayPrice();
  const finalOriginalPrice = originalPrice || computedOriginalPrice;
  
  const hasDiscount = (hasVariantDiscount || (finalOriginalPrice && finalOriginalPrice > displayPrice));
  const discountPercent = hasDiscount
    ? Math.round(((finalOriginalPrice - displayPrice) / finalOriginalPrice) * 100)
    : discount;
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              {hasDiscount && (
                <span className="text-sm text-text-tertiary line-through">
                  {finalOriginalPrice.toLocaleString('vi-VN')}₫
                </span>
              )}
            </div>
